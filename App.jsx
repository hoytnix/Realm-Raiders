...
import { SEASONS, WEATHER_CONDITIONS, SEASON_ORDER, FACTIONS, BUILDINGS, EMPTY_PLOT, DEFAULT_STATE, TECHNOLOGIES, TECH_CATEGORIES, SoundController, sounds, ISO_W, ISO_H, gridToParchmentIso, getBuildingUpgradeCost } from './src/constants/index.js';
import { useGameState, usePlayerStats, useDeskTilt, useRaidBattle, useHotkeys } from './src/hooks/index.js';
import { WaxStampCursor, WaxSplats } from './src/components/common/index.js';
import { MobileTechCodexModal } from './src/components/technology/index.js';
import { MonarchHeader, MobileBottomNav, MobileFloatingHUD, MobileFloraSheet, ResourceBar } from './src/components/hud/index.js';
import { ThroneRoomBackground, DeskSurface, ThroneArmrests, SovereignCommandDock, ParchmentFloraDesk } from './src/components/desk/index.js';
import { ParchmentCitadelMap, VillagerRosterModal } from './src/components/citadel/index.js';
import { ParchmentWarCouncil, ParchmentRaidBattlefieldModal, RewardedAdGateModal } from './src/components/combat/index.js';
import { ParchmentChronicleTome } from './src/components/chronicle/index.js';
import { ParchmentMenuView, ParchmentSettingsModal } from './src/components/menu/index.js';
import { ThroneRoomOnboarding } from './src/components/onboarding/index.js';

// Backward compatibility re-exports
export { SEASONS, WEATHER_CONDITIONS, SEASON_ORDER, FACTIONS, BUILDINGS, EMPTY_PLOT, DEFAULT_STATE, TECHNOLOGIES, TECH_CATEGORIES, SoundController, sounds, ISO_W, ISO_H, gridToParchmentIso };
...
export default function App() {
  ...
  useHotkeys({
    ...
    setDeskView: view => {
      if (view === 'war') sounds.playDaggerThrust();else sounds.playCoin();
      setDeskView(view);
    },
    onToggleTech: () => {
      sounds.playCoin();
  ...
  const onBulkUpgrade = useCallback(ids => {
    handleBulkUpgradeBuildings(ids, currentFaction, () => {
      triggerWaxSplat(typeof window !== 'undefined' ? window.innerWidth * 0.72 : 400, typeof window !== 'undefined' ? window.innerHeight * 0.5 : 300);
    });
  ...
  const onResearchTechnology = useCallback((techId, e) => {
    handleResearchTechnology(techId, () => {
      triggerWaxSplat(e?.clientX || (typeof window !== 'undefined' ? window.innerWidth / 2 : 200), e?.clientY || (typeof window !== 'undefined' ? window.innerHeight / 2 : 200));
    });
  ...
  const onStrike = useCallback(targetKey => {
    handleCatapultStrike(targetKey, gameState.faction, gameState.timeState, (rival, lootGained) => recordRaidVictory(rival, lootGained, stats.caps));
  ...
  const readyHarvestsCount = gameState.grid && Array.isArray(gameState.grid) && gameState.grid.some(p => p.buildingId) ? gameState.grid.filter(p => {
  ...
  }).length : Object.keys(BUILDINGS).filter(bId => {
  ...
  const canUpgradeSelected = !isEmptyPlot && gameState.resources.gold >= costGold && gameState.resources.wood >= costWood && gameState.resources.stone >= costStone;
  ...
  const currentKeepLvl = (gameState.grid && Array.isArray(gameState.grid) ? gameState.grid.find(p => p.buildingId === 'keep')?.level : null) || gameState.buildings?.keep || 1;
  const canResearchAny = Object.values(TECHNOLOGIES).some(tech => {
    ...
    return (gameState.resources?.gold || 0) >= cGold && (gameState.resources?.food || 0) >= cFood && (gameState.resources?.wood || 0) >= cWood && (gameState.resources?.stone || 0) >= cStone;
  });
  return <div onMouseMove={handlePointerMove} className={`relative w-full h-[100dvh] overflow-hidden bg-stone-950 font-serif select-none flex flex-col justify-between ${screenShake ? 'animate-bounce' : ''}`}>
      {/* DIEGETIC WAX STAMP CURSOR */}
      <WaxStampCursor isVisible={isHoveringUpgradeable || isStampCursorEquipped} cursorPos={cursorPos} stampingDecree={stampingDecree} />

      {/* Ephemeral Stamped Crimson Wax Splats */}
      <WaxSplats splats={waxSplats} />

      {/* LAYER 1: UPPER BACKGROUND (Parallax Masonry Arch & God-Rays) */}
      <ThroneRoomBackground tilt={tilt} isStarving={isStarving} currentFaction={currentFaction} candlelitMode={candlelitMode} onToggleCandlelight={() => setCandlelitMode(v => !v)} />

      {/* LAYER 2A: DESKTOP DIEGETIC STATUS HUD (Floating Gilded Crown Bar with Persistent ResourceBar) */}
      <MonarchHeader currentFaction={currentFaction} stats={stats} timeState={timeState} resources={gameState.resources} isMuted={isMuted} isStarving={isStarving} starvationDeaths={starvationDeaths} brambleShieldActive={brambleShieldActive} onToggleBrambleShield={toggleBrambleShield} onTriggerVerdantBloom={triggerVerdantBloom} onToggleMute={() => setIsMuted(!isMuted)} onOpenSettings={() => setIsSettingsOpen(true)} onRaidGrain={() => setDeskView('war')} onForage={forageEmergencyRations} canForage={canForage} forageCooldownSec={forageCooldownSec} />

      {/* LAYER 2B: MOBILE FLOATING HUD (< md Minimal Chronometer) */}
      <MobileFloatingHUD timeState={timeState} deskView={deskView} />

      {/* LAYER 2C: MOBILE RESOURCE HUD DOCKED DIRECTLY ABOVE THE ROYAL LEDGER (Exclusively on Citadel tab) */}
      {deskView === 'citadel' && <div className={`md:hidden fixed left-0 right-0 z-30 px-2 pointer-events-none flex justify-center transition-all duration-300 ease-out ${isInspectorExpanded ? 'bottom-[calc(75vh+3.5rem+4px)]' : 'bottom-[calc(3.5rem+3rem+4px)]'}`}>
          <div className="pointer-events-auto max-w-full">
            <ResourceBar resources={gameState.resources} stats={stats} variant="mobile" />
          </div>
        </div>}
      ...
      <DeskSurface tilt={tilt} isStarving={isStarving} gameState={gameState} stats={stats} currentFaction={currentFaction} deskView={deskView} setDeskView={setDeskView} onToggleStampCursor={() => setIsStampCursorEquipped(v => !v)} isStampCursorEquipped={isStampCursorEquipped}>
        ...
        {deskView === 'flora' && <ParchmentFloraDesk stats={stats} resources={gameState.resources} timeState={timeState} brambleShieldActive={brambleShieldActive} onToggleBrambleShield={toggleBrambleShield} onTriggerVerdantBloom={triggerVerdantBloom} onTransmuteFlora={transmuteFlora} currentFaction={currentFaction} />}

        {deskView === 'chronicle' && <div className="h-full min-h-0 flex-1 flex flex-col w-full">
            <ParchmentChronicleTome logs={gameState.battleLogs} state={gameState} onResetSave={onResetSave} />
          </div>}

        {deskView === 'menu' && <ParchmentMenuView currentFaction={currentFaction} stats={stats} state={gameState} settings={settings} onUpdateSettings={handleUpdateSettings} isSettingsOpen={isSettingsOpen} onOpenSettings={() => setIsSettingsOpen(true)} onCloseSettings={() => setIsSettingsOpen(false)} onNavigate={onNavigateMenu} onResetSave={onResetSave} />}
      </DeskSurface>

      {/* LAYER 4: LOWER FOREGROUND (The Royal Throne & Monarch Hands - Desktop only) */}
      <ThroneArmrests tilt={tilt} currentFaction={currentFaction} />

      {/* LAYER 5: SOVEREIGN COMMAND DOCK (Desktop Centered Floating Navigation) */}
      <SovereignCommandDock deskView={deskView} setDeskView={setDeskView} hasUnavengedFeuds={hasUnavengedFeuds} readyHarvestsCount={readyHarvestsCount} onOpenFlora={() => setDeskView('flora')} isFloraOpen={deskView === 'flora'} />

      {/* REALM SETTINGS & AUDIO MODAL */}
      <ParchmentSettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} settings={settings} onUpdateSettings={handleUpdateSettings} onResetSave={onResetSave} />

      {/* MOBILE TECH CODEX & ROYAL DECREES MODAL */}
      <MobileTechCodexModal isOpen={isTechCodexOpen} onClose={() => setIsTechCodexOpen(false)} technologies={gameState.technologies || []} unlockedTech={unlockedTech || gameState.technologies || []} currentResearch={currentResearch || gameState.currentResearch || null} researchProgress={researchProgress || 0} onResearchTech={researchTech} onResearchTechnology={onResearchTechnology} researchTech={researchTech} resources={gameState.resources} buildings={gameState.buildings} grid={gameState.grid} currentFaction={currentFaction} />

      {/* MOBILE FLORA ELEMENTAL MANAGEMENT SHEET (Strictly Mobile < md) */}
      <div className="md:hidden">
        <MobileFloraSheet isOpen={isFloraSheetOpen || deskView === 'flora'} onClose={() => {
        setIsFloraSheetOpen(false);
        if (deskView === 'flora') setDeskView('citadel');
      }} stats={stats} resources={gameState.resources} timeState={timeState} brambleShieldActive={brambleShieldActive} onToggleBrambleShield={toggleBrambleShield} onTriggerVerdantBloom={triggerVerdantBloom} onTransmuteFlora={transmuteFlora} onNavigate={view => {
        setIsFloraSheetOpen(false);
        setDeskView(view);
      }} />
      </div>
      ...
      <MobileBottomNav deskView={deskView} setDeskView={setDeskView} hasUnavengedFeuds={hasUnavengedFeuds} readyHarvestsCount={readyHarvestsCount} canUpgradeSelected={canUpgradeSelected} selectedBuildingName={selectedDef.name} isCodexOpen={isTechCodexOpen} onOpenCodex={() => {
      setIsFloraSheetOpen(false);
      setIsTechCodexOpen(true);
    }} onCloseCodex={() => setIsTechCodexOpen(false)} isFloraOpen={isFloraSheetOpen || deskView === 'flora'} onOpenFlora={() => {
      setIsTechCodexOpen(false);
      setIsFloraSheetOpen(true);
    }} onCloseFlora={() => {
      setIsFloraSheetOpen(false);
      if (deskView === 'flora') setDeskView('citadel');
    }} isResearching={isResearching} canResearchAny={canResearchAny} resources={gameState.resources} stats={stats} onHarvestAll={() => handleHarvestAll(stats.caps, currentFaction)} onUpgradeSelected={e => onUpgradeBuilding(selectedBuildingId, e)} onTriggerStamp={e => triggerWaxSplat(e?.clientX || window.innerWidth / 2, e?.clientY || window.innerHeight / 2)} />

      {/* REWARDED VIDEO AD GATE SIMULATOR */}
      {adModalOpen && <RewardedAdGateModal countdown={adCountdown} targetRival={pendingRaidTarget} onComplete={confirmAdAndLaunchPillage} onDevSkip={confirmAdAndLaunchPillage} />}

      {/* ACTIVE 2.5D CATAPULT RAID */}
      {activeRaid && <ParchmentRaidBattlefieldModal raid={activeRaid} onStrike={onStrike} onDoubleLoot={onDoubleLoot} onClose={closeRaid} />}

      {/* ROYAL VILLAGER ROSTER MODAL */}
      <VillagerRosterModal isOpen={isRosterOpen} onClose={() => setIsRosterOpen(false)} villagers={gameState.villagers || []} grid={gameState.grid || []} onAssignVillager={assignVillager} onUnassignVillager={unassignVillager} isStarving={isStarving} isDehydrated={isDehydrated} />
    </div>;
}