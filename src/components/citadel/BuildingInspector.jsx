...
import { BUILDINGS, EMPTY_PLOT, CONSTRUCTIBLE_BLUEPRINTS, TERRITORY_TIERS, MAX_TERRITORY_TIER, TROOP_RECRUIT_COST, toRomanTier, getTierEpoch, formatBuildDuration, calculateBuildingYield, calculateBuildDuration, sounds } from '../../constants/index.js';
...
export function BuildingInspector({
  ...
  const canAffordLogistics = !hasTroopLogistics && currentLvl >= 1 && (resources.gold || 0) >= 40 && (resources.food || 0) >= 30;
  const toggleExpand = () => {
    haptics.light();
  ...
  const canAnnexTerritory = nextTierDef && currentLvl >= nextTierDef.keepLevelReq && resources.gold >= nextTierDef.cost.gold && resources.wood >= nextTierDef.cost.wood && resources.stone >= nextTierDef.cost.stone;

  // Selected Blueprint data
  const currentBlueprint = BUILDINGS[selectedBlueprintKey] || BUILDINGS.farm;
  const canAffordConstruct = resources.gold >= (currentBlueprint.baseCost?.gold || 0) && resources.wood >= (currentBlueprint.baseCost?.wood || 0) && resources.stone >= (currentBlueprint.baseCost?.stone || 0);
  ...
  const canRecruit = resources.gold >= recruitCostGold && resources.food >= recruitCostFood && troops.total + recruitCount <= maxTroopCap;
  return <>
      {/* =========================================================================
          THE ROYAL LEDGER (CITADEL INSPECTOR): MOBILE BOTTOM SHEET (< md)
          ========================================================================= */}
      <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className={`md:hidden fixed bottom-14 left-0 right-0 z-30 bg-[#e4d4b3] border-t-2 border-[#bfa379] shadow-[0_-10px_25px_rgba(0,0,0,0.4)] transition-all duration-300 ease-out flex flex-col ${isExpanded ? 'h-[75vh] p-3 overflow-hidden' : 'h-12 py-1 px-3 overflow-hidden'}`}>
        {/* Drag Handle Bar & Peek Header */}
        <div onClick={toggleExpand} className="w-full flex items-center justify-between cursor-pointer py-1 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                ...
                {!isEmptyPlot && <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-[#6b4724] text-amber-200 font-bold">
                    T{currentLvl}
                  </span>}
                {isAutomated && <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-emerald-900/20 text-emerald-900 font-bold border border-emerald-700/40 flex items-center gap-0.5">
                    🛡️ Auto
                  </span>}
              </div>
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-mono text-[#6b4a2e] font-bold">
            {!isEmptyPlot && !isExpanded && (canAfford ? <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-green-900/20 text-green-900 font-bold border border-green-700/40 animate-pulse">
                  Ready ✨
                </span> : <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-amber-900/10 text-amber-900 font-bold">
                  🪙{costGold}
                </span>)}
        ...
        {/* Expanded Sheet Body */}
        {isExpanded && <div className="flex-1 overflow-y-auto overscroll-contain mt-2 pt-2 border-t border-[#bfa379]/60 space-y-3 animate-in fade-in duration-200 pb-4">
            {/* MODE 1: EMPTY FOUNDATION CAROUSEL */}
            {isEmptyPlot ? <div className="space-y-2">
              ...
              return <button key={bpKey} onClick={() => {
                sounds.playCoin();
                haptics.light();
                setSelectedBlueprintKey(bpKey);
              }} className={`p-2 rounded-xl border flex flex-col items-center min-w-[76px] transition flex-shrink-0 ${isSelectedBp ? 'bg-[#cbb38b] border-[#8c6843] shadow-md ring-2 ring-amber-600' : 'bg-[#dfcba6] border-[#bfa379] hover:bg-[#d5be97]'}`}>
                ...
                <button onClick={() => onConstructBuilding && onConstructBuilding(selectedBuildingId, selectedBlueprintKey)} disabled={!canAffordConstruct} className={`w-full py-3 rounded-xl font-black text-xs transition flex items-center justify-center gap-2 shadow-lg ${canAffordConstruct ? 'bg-gradient-to-r from-amber-800 to-yellow-800 text-amber-100 active:scale-95 shadow-amber-950/40 border border-amber-600' : 'bg-stone-400 text-stone-700 cursor-not-allowed border border-stone-500'}`}>
                  <span>🏗️</span>
                  <span>Erect Structure (Commission {currentBlueprint.name})</span>
                </button>
              </div> : (/* MODE 2: CONSTRUCTED STRUCTURE INSPECTION & UPGRADE */
        <div className="space-y-3">
                ...
                {isUpgrading && <div className="bg-[#dfcba6] p-3 rounded-2xl border-2 border-amber-800 shadow-md space-y-2 animate-in fade-in">
                    ...
                    <div className="w-full bg-stone-900/20 rounded-full h-3 overflow-hidden border border-[#8c6843]/60 p-0.5">
                      <div className="bg-gradient-to-r from-amber-700 via-yellow-600 to-amber-500 h-full rounded-full transition-all duration-300 relative overflow-hidden" style={{
                width: `${Math.min(100, Math.max(5, Math.round((1 - upgradeTimeRemaining / (totalUpgradeTime || 1)) * 100)))}%`
              }}>
                ...
                {isWorkerBuilding && <div className={`p-3 rounded-2xl border-2 shadow-md space-y-2 ${isUnstaffed ? 'bg-amber-950/20 border-amber-600/80 text-amber-950' : 'bg-[#dfcba6] border-[#8c6843]'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-[9.5px] font-mono font-bold px-2 py-0.5 rounded border ${isUnstaffed ? 'bg-amber-800/25 text-amber-950 border-amber-800/60 animate-pulse' : 'bg-emerald-800/20 text-emerald-950 border-emerald-800/40'}`}>
                        {isUnstaffed ? '⚠️ IDLE (UNSTAFFED)' : `⚡ Output: ${(0.6 + 0.4 * assignedWorkers.length).toFixed(1)}x`}
                      </span>
                    </div>

                    {isUnstaffed ? <p className="text-[10px] font-mono text-amber-900 leading-tight">
                        Production cycle timer is halted. Assign at least 1 citizen to begin generating yields.
                      </p> : <div className="flex items-center gap-1.5 flex-wrap">
                        {assignedWorkers.map(w => <span key={w.id} className="px-2 py-0.5 rounded-lg bg-[#ebdcc1] border border-[#8c6843]/60 text-[10px] font-mono font-bold text-[#3f2314] flex items-center gap-1 shadow-sm">
                            <span>👤</span>
                            <span>{w.name}</span>
                          </span>)}
                      </div>}
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button onClick={() => onAssignWorker && onAssignWorker(inspectPlotId)} disabled={assignedWorkers.length >= maxWorkers} className={`flex-1 py-1.5 px-2 rounded-xl font-mono text-xs font-bold transition flex items-center justify-center gap-1 shadow ${assignedWorkers.length < maxWorkers ? 'bg-gradient-to-r from-amber-800 to-yellow-800 text-amber-100 hover:brightness-105 active:scale-95 border border-amber-600' : 'bg-stone-400/50 text-stone-600 cursor-not-allowed border border-stone-400'}`}>
                        <span>+</span>
                        <span>Assign Villager</span>
                      </button>

                      <button onClick={() => onUnassignWorker && onUnassignWorker(inspectPlotId)} disabled={assignedWorkers.length === 0} className={`py-1.5 px-3 rounded-xl font-mono text-xs font-bold transition flex items-center justify-center gap-1 shadow ${assignedWorkers.length > 0 ? 'bg-[#ebdcc1] hover:bg-[#dfcba6] text-[#6b4a2e] border border-[#8c6843] active:scale-95' : 'bg-stone-300/50 text-stone-500 cursor-not-allowed border border-stone-300'}`} title="Unassign 1 Worker">
                        <span>−</span>
                        <span>Unassign</span>
                      </button>

                      {onOpenRoster && <button onClick={onOpenRoster} className="py-1.5 px-2.5 rounded-xl bg-[#ebdcc1] hover:bg-[#dfcba6] text-[#3f2314] border border-[#8c6843] text-xs font-mono font-bold transition shadow active:scale-95" title="Open Royal Citizen Roster">
                          📜 Roster
                        </button>}
                    </div>
                  </div>}
                ...
                <div className="bg-[#dfcba6] p-3 rounded-2xl border-2 border-[#8c6843] shadow-md space-y-2.5">
                  ...
                  <div className="bg-[#ebdcc1] p-2 rounded-xl border border-[#bfa379] space-y-1 text-xs font-mono">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-amber-950">
                        {isHarvestBuilding ? `${currYield}/cycle → ${nextYield}/cycle` : isKeep ? `${currDef} HP → ${nextDef} HP` : isBarracks ? `${currTroopCap} Troops → ${nextTroopCap} Troops` : `${currTowerDef} Def → ${nextTowerDef} Def`}
                      </span>
                    </div>
                  </div>

                  {/* Deep Vault Storage Gating Warning */}
                  {isVaultGated && <div className="bg-red-950/20 p-2 rounded-xl border-2 border-red-800/60 text-red-950 text-xs font-mono flex items-start gap-2 animate-pulse">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                    <div className={`p-2 rounded-xl border text-center ${resources.gold >= costGold ? 'bg-yellow-900/10 border-yellow-800/40 text-yellow-900 font-bold' : 'bg-red-900/15 border-red-800/60 text-red-900 font-bold'}`}>
                    ...
                    <div className={`p-2 rounded-xl border text-center ${resources.wood >= costWood ? 'bg-orange-900/10 border-orange-800/40 text-orange-900 font-bold' : 'bg-red-900/15 border-red-800/60 text-red-900 font-bold'}`}>
                      <div className="text-xs">🪵 {costWood}</div>
                      <div className="text-[9px] font-sans opacity-80 mt-0.5">
                        {resources.wood >= costWood ? 'Timber' : `(-${costWood - (resources.wood || 0)})`}
                      </div>
                    </div>
                    <div className={`p-2 rounded-xl border text-center ${resources.stone >= costStone ? 'bg-stone-900/10 border-stone-800/40 text-stone-900 font-bold' : 'bg-red-900/15 border-red-800/60 text-red-900 font-bold'}`}>
              ...
              handleUpgradeClick(e);
            }} disabled={!canAfford || stampingDecree || isUpgrading || isVaultGated || isMaxTier} className={`w-full py-3 px-4 rounded-xl font-black text-xs transition flex items-center justify-center gap-2 border-2 shadow-xl ${isUpgrading ? 'bg-amber-950/20 text-amber-900 border-amber-800/50 cursor-not-allowed' : isMaxTier ? 'bg-stone-300 text-stone-600 border-stone-400 cursor-not-allowed' : isVaultGated ? 'bg-red-950/20 text-red-900 border-red-800/60 cursor-not-allowed' : canAfford ? 'bg-gradient-to-r from-red-800 via-rose-800 to-red-700 text-amber-100 hover:brightness-110 active:scale-95 border-red-600 shadow-red-950/40 cursor-pointer' : 'bg-stone-300 text-stone-600 border-stone-400 cursor-not-allowed'}`}>
                    <span className={`text-base ${stampingDecree ? 'animate-spin' : ''}`}>
                      {isUpgrading ? '🧱' : isVaultGated ? '🔒' : '🩸'}
                    </span>
                    <span className="tracking-wide uppercase">
                      {isUpgrading ? `Masonry Underway (${formatBuildDuration(upgradeTimeRemaining)})` : isMaxTier ? `Monumental Tier XX Reached` : isVaultGated ? `Vault Capacity Deficient` : canAfford ? `SEAL ROYAL UPGRADE (Tier ${toRomanTier(currentLvl + 1)})` : `Insufficient Resources for Tier ${toRomanTier(currentLvl + 1)}`}
                    </span>
                  </button>
                </div>

                {/* BARRACKS / GARRISON RECRUITMENT ACTION */}
                {isBarracks && <div className="bg-[#dfcba6] p-2.5 rounded-2xl border-2 border-amber-800/40 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                  ...
                  haptics.light();
                  setRecruitCount(cnt);
                }} className={`px-2.5 py-1 rounded-lg text-xs font-mono font-bold border transition ${recruitCount === cnt ? 'bg-amber-900 text-amber-100 border-amber-950' : 'bg-[#ebdcc1] text-[#442813] border-[#8c6843]'}`}>
                            +{cnt}
                          </button>)}
                        <button onClick={() => {
                  sounds.playCoin();
              ...
              }} disabled={!canRecruit} className={`px-3.5 py-1.5 rounded-xl font-black text-xs transition flex items-center gap-1.5 shadow ${canRecruit ? 'bg-gradient-to-r from-red-800 to-rose-900 text-amber-100 hover:brightness-110 active:scale-95 border border-red-700' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                        <span>⚔️</span>
                ...
                {isHarvestBuilding && isAutomated && <div className="bg-emerald-900/10 border border-emerald-700/40 rounded-xl p-2 flex items-center gap-2">
                    <span className="text-base">🛡️</span>
                ...
                {isKeep && <div className="bg-[#dfcba6] p-2.5 rounded-xl border border-amber-800/40 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                      ...
                      {hasTroopLogistics ? <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded bg-emerald-800 text-emerald-100 border border-emerald-600">
                          Sealed 🛡️
                        </span> : <span className={`text-[9px] font-mono font-bold px-1.5 py-0.5 rounded ${currentLvl >= 2 ? 'bg-green-900/10 text-green-900' : 'bg-red-900/10 text-red-900'}`}>
                          Req Keep T2
                        </span>}
                    </div>
                    ...
                    {!hasTroopLogistics && <div className="flex items-center justify-between pt-1">
                        ...
                        <button onClick={() => onResearchTechnology && onResearchTechnology('tech_troop_logistics')} disabled={!canAffordLogistics} className={`px-3 py-1 rounded-lg text-xs font-black transition ${canAffordLogistics ? 'bg-gradient-to-r from-red-800 to-rose-900 text-amber-100 hover:brightness-110 active:scale-95 shadow border border-red-700' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                ...
                {isKeep && nextTierDef && <div className="bg-[#dfcba6] p-2.5 rounded-xl border border-amber-800/40 space-y-2">
                    <p className="text-[10px] text-[#6b4a2e]">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => onExpandTerritory && onExpandTerritory()} disabled={!canAnnexTerritory} className={`px-3 py-1 rounded-lg text-xs font-black transition ${canAnnexTerritory ? 'bg-gradient-to-r from-amber-800 to-yellow-800 text-amber-100 active:scale-95 border border-amber-600' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                        Annex
                      </button>
                    </div>
                  </div>}
                ...
                {isVault && <div className="bg-[#dfcba6] p-2.5 rounded-xl border border-amber-800/40 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <span className="text-base">⚗️</span>
                        <div>
                    ...
                    <div className="space-y-1.5 pt-1">
                      ...
                      <div className="bg-[#ebdcc1] p-2 rounded-xl border border-[#bfa379] flex items-center justify-between gap-2">
                        ...
                        <button onClick={() => onTransmuteFlora && onTransmuteFlora('stone')} disabled={(resources.flora || 0) < 100 || (resources.food || 0) < 50} className={`px-2.5 py-1 rounded-lg text-xs font-bold transition shadow-sm ${(resources.flora || 0) >= 100 && (resources.food || 0) >= 50 ? 'bg-stone-800 hover:bg-stone-900 text-amber-100 active:scale-95 border border-stone-700' : 'bg-stone-300 text-stone-500 cursor-not-allowed'}`}>
                          Transmute
                        </button>
                      </div>
                      ...
                      <div className="bg-[#ebdcc1] p-2 rounded-xl border border-[#bfa379] flex items-center justify-between gap-2">
                        ...
                        <button onClick={() => onTransmuteFlora && onTransmuteFlora('gold')} disabled={(resources.flora || 0) < 100 || (resources.food || 0) < 25} className={`px-2.5 py-1 rounded-lg text-xs font-bold transition shadow-sm ${(resources.flora || 0) >= 100 && (resources.food || 0) >= 25 ? 'bg-amber-800 hover:bg-amber-900 text-amber-100 active:scale-95 border border-amber-600' : 'bg-stone-300 text-stone-500 cursor-not-allowed'}`}>
                          Distill
                        </button>
                      </div>
                    </div>
      ...
      <div className="hidden md:flex bg-[#e4d4b3] border-t-2 border-[#bfa379] p-3 items-center justify-between gap-3">
        {isEmptyPlot ? (/* EMPTY FOUNDATION BLUEPRINT DRAWER */
      <div className="w-full flex items-center justify-between gap-3">
            ...
            return <button key={bpKey} onClick={() => {
              ...
              haptics.light();
              setSelectedBlueprintKey(bpKey);
            }} className={`px-2.5 py-1.5 rounded-xl border flex items-center gap-1.5 transition ${isSelectedBp ? 'bg-[#cbb38b] border-[#8c6843] shadow-md ring-2 ring-amber-600' : 'bg-[#dfcba6] border-[#bfa379] hover:bg-[#d5be97]'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => onConstructBuilding && onConstructBuilding(selectedBuildingId, selectedBlueprintKey)} disabled={!canAffordConstruct} className={`px-4 py-2 rounded-xl font-black text-xs transition flex items-center gap-2 shadow-lg ${canAffordConstruct ? 'bg-gradient-to-r from-amber-800 to-yellow-800 text-amber-100 hover:brightness-110 active:scale-95 shadow-amber-950/40 border border-amber-600' : 'bg-stone-400 text-stone-700 cursor-not-allowed border border-stone-500'}`}>
                <span>🏗️</span>
                <span>Erect {currentBlueprint.name}</span>
              </button>
            </div>
          </div>) : (/* CONSTRUCTED BUILDING CONTROLS & SPECIAL ACTIONS */
      <div className="w-full flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-[#cbb38b] border border-[#8c6843] flex items-center justify-center text-2xl shadow-inner flex-shrink-0">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {isUpgrading && <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-900/20 text-amber-900 font-bold border border-amber-700/40 flex items-center gap-1 animate-pulse">
                      🧱 Inscribing Tier {toRomanTier(targetTier)} ({formatBuildDuration(upgradeTimeRemaining)})
                    </span>}
                  {isAutomated && <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-emerald-900/20 text-emerald-900 font-bold border border-emerald-700/40 flex items-center gap-1">
            ...
            {isKeep && <div className="bg-[#dfcba6] px-2.5 py-1.5 rounded-xl border border-[#bfa379] flex items-center gap-2 text-xs">
                <div>
                ...
                {hasTroopLogistics ? <span className="px-2 py-1 rounded-lg text-[10px] font-mono font-bold bg-emerald-800 text-emerald-100 border border-emerald-600">
                    Sealed 🛡️
                  </span> : <button onClick={() => onResearchTechnology && onResearchTechnology('tech_troop_logistics')} disabled={!canAffordLogistics} className={`px-2.5 py-1 rounded-lg text-xs font-black transition ${canAffordLogistics ? 'bg-gradient-to-r from-red-800 to-rose-900 text-amber-100 hover:brightness-110 active:scale-95 shadow border border-red-700' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                    Seal Decree
                  </button>}
              </div>}

            {/* KEEP SPECIAL ACTION: ANNEX TERRITORY */}
            {isKeep && nextTierDef && <div className="bg-[#dfcba6] px-2.5 py-1.5 rounded-xl border border-[#bfa379] flex items-center gap-2 text-xs">
                <div>
                ...
                <button onClick={() => onExpandTerritory && onExpandTerritory()} disabled={!canAnnexTerritory} className={`px-2.5 py-1 rounded-lg text-xs font-black transition ${canAnnexTerritory ? 'bg-amber-800 text-amber-100 hover:brightness-110 active:scale-95 shadow' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                  Annex
                </button>
              </div>}

            {/* BARRACKS SPECIAL ACTION: RECRUIT LEVIES */}
            {isBarracks && <div className="bg-[#dfcba6] px-2.5 py-1.5 rounded-xl border border-[#bfa379] flex items-center gap-2 text-xs">
                <div>
                ...
                <div className="flex gap-1">
                  {[1, 5].map(cnt => <button key={cnt} onClick={() => setRecruitCount(cnt)} className={`px-1.5 py-0.5 rounded text-[10px] font-mono font-bold ${recruitCount === cnt ? 'bg-amber-900 text-amber-100' : 'bg-[#cbb38b]'}`}>
                      +{cnt}
                    </button>)}
                  <button onClick={() => {
              const maxAffordable = Math.floor((resources.gold || 0) / 25);
            ...
            triggerHaptic('heavy');
            if (onTrainTroops) onTrainTroops(recruitCount);
          }} disabled={!canRecruit} className={`px-2.5 py-1 rounded-lg text-xs font-black transition ${canRecruit ? 'bg-red-800 text-amber-100 hover:brightness-110 active:scale-95 shadow' : 'bg-stone-400 text-stone-600 cursor-not-allowed'}`}>
                  Enlist
                </button>
              </div>}

            {/* VAULT SPECIAL ACTION: ALCHEMICAL TRANSMUTATION */}
            {isVault && <div className="bg-[#dfcba6] px-2.5 py-1.5 rounded-xl border border-[#bfa379] flex items-center gap-2 text-xs">
                <div>
                  <div className="font-bold text-[#442813] text-[11px] flex items-center gap-1">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => onTransmuteFlora && onTransmuteFlora('stone')} disabled={(resources.flora || 0) < 100 || (resources.food || 0) < 50} className={`px-2 py-1 rounded text-[10px] font-bold transition ${(resources.flora || 0) >= 100 && (resources.food || 0) >= 50 ? 'bg-stone-800 text-amber-100 hover:bg-stone-900 border border-stone-700 active:scale-95' : 'bg-stone-300 text-stone-500 cursor-not-allowed'}`} title="100 Flora + 50 Sustenance -> 50 Stone">
                    🪨 Granite (+50)
                  </button>
                  <button onClick={() => onTransmuteFlora && onTransmuteFlora('gold')} disabled={(resources.flora || 0) < 100 || (resources.food || 0) < 25} className={`px-2 py-1 rounded text-[10px] font-bold transition ${(resources.flora || 0) >= 100 && (resources.food || 0) >= 25 ? 'bg-amber-800 text-amber-100 hover:bg-amber-900 border border-amber-600 active:scale-95' : 'bg-stone-300 text-stone-500 cursor-not-allowed'}`} title="100 Flora + 25 Sustenance -> 40 Gold">
                    🪙 Tincture (+40)
                  </button>
                </div>
              </div>}
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={e => onUpgradeBuilding && onUpgradeBuilding(selectedBuildingId, e)} onMouseEnter={() => {
            if (canAfford) onHoverUpgrade && onHoverUpgrade(true);
          }} onMouseLeave={() => onHoverUpgrade && onHoverUpgrade(false)} disabled={!canAfford || stampingDecree} className={`px-4 py-2 rounded-xl font-black text-xs transition flex items-center gap-2 shadow-lg ${canAfford ? 'bg-gradient-to-r from-red-800 to-rose-900 text-amber-100 hover:brightness-110 active:scale-95 shadow-red-950/40 border border-red-700' : 'bg-stone-400 text-stone-700 cursor-not-allowed border border-stone-500'}`}>
