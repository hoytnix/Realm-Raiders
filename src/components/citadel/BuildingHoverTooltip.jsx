...
export function BuildingHoverTooltip({
  ...
  const {
    ...
    wood: costWood,
    stone: costStone
  } = getBuildingUpgradeCost(bDef, level, discount);
  const canAfford = (resources.gold || 0) >= costGold && (resources.wood || 0) >= costWood && (resources.stone || 0) >= costStone;
  ...
  return <div className="fixed z-50 pointer-events-none transition-opacity duration-150 animate-in fade-in zoom-in-95" style={{
    left: `${left}px`,
    top: `${top}px`
  }}>
      <div className="w-56 bg-[#f4ecd8] border-2 border-[#8c6843] rounded-xl shadow-2xl p-2.5 text-stone-900 font-serif">
        ...
        {yieldInfo && <div className="bg-[#dfcba6]/60 p-1.5 rounded-lg border border-[#bfa379]/40 mb-1.5 text-[10px] font-mono">
        ...
        {/* Automation Status */}
        {bDef.cycleDuration && <div className="mb-1.5 flex items-center gap-1 text-[9px] font-mono">
            {isAutomated ? <span className="text-emerald-800 font-bold flex items-center gap-1 bg-emerald-900/10 px-1.5 py-0.5 rounded border border-emerald-700/30">
                🛡️ Garrison Auto-Reaping
              </span> : <span className="text-amber-900 flex items-center gap-1 bg-amber-900/10 px-1.5 py-0.5 rounded">
        ...
        <div className="border-t border-[#bfa379]/60 pt-1">
          <div className="flex items-center justify-between text-[9px] font-mono text-[#6b4a2e] mb-0.5">
            ...
            {canAfford ? <span className="text-green-800 font-bold">Affordable ✨</span> : <span className="text-red-800 font-bold">Short Stores</span>}
          </div>
