...
import { BUILDINGS, ISO_W, ISO_H, MAX_GRID_SIZE, gridToParchmentIso, gridToIso, rotateGridCoords, isPlotAnnexed, createDefaultGrid, toRomanTier, sounds } from '../../constants/index.js';
...
export function CitadelSvgGrid({
  ...
  const keepRot = keepPlot ? rotateGridCoords(keepPlot.gx, keepPlot.gy, rotationAngle, MAX_GRID_SIZE) : rotateGridCoords(1, 1, rotationAngle, MAX_GRID_SIZE);
  const wellRot = wellPlot ? rotateGridCoords(wellPlot.gx, wellPlot.gy, rotationAngle, MAX_GRID_SIZE) : rotateGridCoords(2, 1, rotationAngle, MAX_GRID_SIZE);
  const granaryRot = granaryPlot ? rotateGridCoords(granaryPlot.gx, granaryPlot.gy, rotationAngle, MAX_GRID_SIZE) : rotateGridCoords(0, 1, rotationAngle, MAX_GRID_SIZE);
  ...
  for (let gx = 0; gx < MAX_GRID_SIZE; gx++) {
    for (let gy = 0; gy < MAX_GRID_SIZE; gy++) {
      const {
        gxPrime,
        gyPrime
      } = rotateGridCoords(gx, gy, rotationAngle, MAX_GRID_SIZE);
      const {
        x,
        y
      } = gridToIso(gxPrime, gyPrime, 270, 48);
  ...
  const annexedPlots = activeGrid.filter(p => isPlotAnnexed(p.gx, p.gy, territoryTier)).map(plot => {
    const {
      gxPrime,
      gyPrime
    } = rotateGridCoords(plot.gx, plot.gy, rotationAngle, MAX_GRID_SIZE);
    const {
      x,
      y
    } = gridToIso(gxPrime, gyPrime, 270, 48);
  ...
  return <div ref={containerRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onContextMenu={e => e.preventDefault()} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} className="flex-1 w-full h-full relative flex items-center justify-center overflow-hidden touch-none select-none">
      {/* Marquee Drag Selection Box Overlay */}
      {marqueeBox && <div className="absolute pointer-events-none border-2 border-amber-600 bg-amber-500/20 rounded z-30" style={{
      ...
      <svg viewBox="0 0 540 330" className="w-full h-full max-h-[360px] md:max-h-full cursor-pointer select-none transition-transform duration-100 ease-out" style={{
      ...
      transformOrigin: 'center center'
        ...
        return <g key={`tile-${gx}-${gy}`}>
              <polygon points={`
                  ${x},${y - ISO_H / 2}
                  ${x + ISO_W / 2},${y}
                  ${x},${y + ISO_H / 2}
                  ${x - ISO_W / 2},${y}
                `} fill={!isAnnexed ? 'url(#parchmentFog)' : isRoad ? '#d5be97' : 'url(#parchmentGround)'} stroke={isAnnexed ? '#8c6843' : '#a88d6a'} strokeWidth={isAnnexed ? '1.2' : '0.9'} strokeDasharray={!isAnnexed ? '3,3' : isRoad ? '2,2' : 'none'} opacity={isAnnexed ? 1 : 0.65} onClick={() => {
            if (!isAnnexed) {
              sounds.playCoin();
              haptics.light();
              ...
              {!isAnnexed && <text x={x} y={y + 4} textAnchor="middle" fill="#78553d" fontSize="11" opacity="0.45" className="select-none pointer-events-none">
                  🔒
                </text>}
            </g>;
      })}
        ...
        {wellPlot && <>
            <line x1={wellIso.x} y1={wellIso.y} x2={keepIso.x} y2={keepIso.y} stroke="#0284c7" strokeWidth="2" strokeDasharray="4,6" strokeDashoffset={-inkPulseTick} opacity="0.8" />
            <line x1={wellIso.x} y1={wellIso.y} x2={granaryIso.x} y2={granaryIso.y} stroke="#0284c7" strokeWidth="2" strokeDasharray="4,6" strokeDashoffset={-inkPulseTick} opacity="0.8" />
          </>}
        ...
        return <g key={village.id} onClick={() => {
          ...
          setActiveNpcVillage(village);
              ...
              <polygon points={`
                  ...
                  ${x},${y + 18}
                  ${x - 36},${y}
                `} fill={village.faction === 'orcs' ? 'rgba(127, 29, 29, 0.28)' : village.faction === 'dwarves' ? 'rgba(120, 53, 15, 0.28)' : 'rgba(3, 105, 161, 0.25)'} stroke={bannerColor} strokeWidth={isHovered ? '2.5' : '1.4'} strokeDasharray="4,2" className="transition-all duration-300 group-hover:stroke-amber-400" />

              {/* Exterior Sentry Patrol Ring Path */}
              <ellipse cx={x} cy={y + 3} rx="28" ry="13" fill="none" stroke="#8c6843" strokeWidth="0.8" strokeDasharray="2,3" opacity="0.5" />
              ...
              <polygon points={`
                  ...
                  ${x},${y + 5}
                  ${x - 14},${y - 2}
                `} fill={village.faction === 'dwarves' ? '#44403c' : village.faction === 'orcs' ? '#450a0a' : '#1e293b'} stroke="#1c1917" strokeWidth="1.2" />
              ...
              <polygon points={`
                  ...
                  ${x - 26},${y - 35}
                  ${x - 12},${y - 30}
                `} fill={bannerColor} stroke="#451a03" strokeWidth="0.8" className="animate-pulse" />
              ...
              <path d={`M ${x + 9.5} ${y - 33} Q ${x + 6} ${y - 42} ${x + 10} ${y - 50}`} stroke="#a8a29e" strokeWidth="1.2" strokeLinecap="round" fill="none" opacity="0.65" className="animate-pulse" />

              {/* Central Sigil Glyph */}
              <text x={x} y={y - 10} textAnchor="middle" fontSize="12" className="select-none pointer-events-none">
                {village.sigil}
              </text>

              {/* Clan Crest Name Tag */}
              <rect x={x - 36} y={y + 11} width="72" height="13" rx="3.5" fill="#1c1917" stroke={bannerColor} strokeWidth="1" opacity="0.92" />
              <text x={x} y={y + 20.5} textAnchor="middle" fill="#f5ecd8" fontSize="6.8" fontWeight="bold" fontFamily="monospace" className="select-none pointer-events-none">
                {village.name}
              </text>
            </g>;
      })}
        ...
        if (!plot.buildingId) {
          ...
          return <g key={plot.id} data-plot-id={plot.id} onClick={() => {
            sounds.playCoin();
            haptics.light();
            onSelectBuilding(plot.id);
                ...
                <polygon points={`
                    ...
                    ${x},${y + ISO_H / 2.5}
                    ${x - ISO_W / 2.5},${y}
                  `} fill={isSelected ? 'rgba(217, 119, 6, 0.28)' : 'rgba(140, 104, 67, 0.12)'} stroke={isSelected ? '#d97706' : '#8c6843'} strokeWidth={isSelected ? '2' : '1.2'} strokeDasharray="4,4" className="transition group-hover:fill-amber-600/20" />

                {/* Stone Foundation Center Marker "+" */}
                <text x={x} y={y + 4} textAnchor="middle" fill={isSelected ? '#78350f' : '#8c6843'} fontSize="13" fontWeight="black" className="select-none pointer-events-none">
                  +
                </text>

                {/* Subtle Plot Coordinate Stamp */}
                <text x={x} y={y - 9} textAnchor="middle" fill={isSelected ? '#78350f' : '#8c6843'} fontSize="8" fontWeight="bold" letterSpacing="0.4" opacity={isSelected ? '1' : '0.65'} className="select-none pointer-events-none font-mono">
                  PLOT
                </text>
              </g>;
        }
        ...
        return <g key={plot.id} data-building-id={plot.id} data-building-type={b.id} data-harvest-ready={isReadyToHarvest ? 'true' : 'false'} onClick={() => {
          if (isReadyToHarvest) {
            haptics.harvest();
            onHarvest(plot.id);
          } else {
          ...
          setHoveredBuilding(null);
              ...
              {multiSelectedIds && (multiSelectedIds.includes(plot.id) || multiSelectedIds.includes(b.id)) && <ellipse cx={x} cy={y} rx={ISO_W / 2 + 6} ry={ISO_H / 2 + 3} fill="rgba(245, 158, 11, 0.22)" stroke="#d97706" strokeWidth="2.5" strokeDasharray="5,3" opacity="0.95" />}
              ...
              {cycleDur > 0 && <ellipse cx={x} cy={y} rx={ISO_W / 2 + 4} ry={ISO_H / 2 + 2} fill="none" stroke={isReadyToHarvest ? '#16a34a' : '#d97706'} strokeWidth={isReadyToHarvest ? '3' : '2'} strokeDasharray="140" strokeDashoffset={140 - progressRatio * 140} opacity="0.85" />}
              ...
              <polygon points={`
                  ...
                  ${x},${y + ISO_H / 3}
                  ${x - ISO_W / 2.5},${y}
                `} fill={isSelected ? '#c2410c' : '#78553d'} stroke="#3f2314" strokeWidth="1.2" />
              ...
              <polygon points={`
                  ...
                  ${x + ISO_W / 2.5},${y}
                  ${x},${y + ISO_H / 3}
                `} fill={isSelected ? '#9a3412' : '#5e3f2b'} stroke="#3f2314" strokeWidth="1.2" />
              ...
              <polygon points={`
                  ...
                  ${x - ISO_W / 2.5},${y - height}
                `} fill={isSelected ? '#f97316' : b.id === 'keep' ? 'url(#roofGoldInk)' : '#9c7349'} stroke="#3f2314" strokeWidth="1.2" />

              {/* Ink Drawn Icon & Tier Seal */}
              <text x={x} y={y - height - 4} textAnchor="middle" className="text-base select-none pointer-events-none">
                {b.inkSymbol}
              </text>
              {/* Ongoing Construction / Masonry Floating Banner */}
              {plot.isUpgrading && <g className="animate-pulse pointer-events-none select-none">
                  <rect x={x - 24} y={y - height - (isReadyToHarvest ? 42 : 24)} width="48" height="14" rx="3" fill="#3f2314" stroke="#f59e0b" strokeWidth="1.2" />
                  <text x={x} y={y - height - (isReadyToHarvest ? 32 : 14)} textAnchor="middle" fill="#fef08a" fontSize="7.5" fontWeight="bold" fontFamily="monospace">
                    🧱 Tier {toRomanTier(plot.targetTier || level + 1)}
                  </text>
                </g>}

              <rect x={x - (isAutomated ? 22 : 16)} y={y - height + 8} width={isAutomated ? 44 : 32} height="12" rx="3" fill="#3f2314" stroke={isAutomated ? '#22c55e' : '#eab308'} strokeWidth={isAutomated ? '1.5' : '1'} />
              <text x={x} y={y - height + 17} textAnchor="middle" fill={isAutomated ? '#86efac' : '#fef08a'} fontSize="8" fontWeight="bold" fontFamily="monospace">
                {isAutomated ? `🛡️ ${toRomanTier(level)}` : `Tier ${toRomanTier(level)}`}
              </text>

              {/* Click-to-Collect Floating Ink Banner */}
              {isReadyToHarvest && <g className="animate-bounce">
                  <rect x={x - 38} y={y - height - 24} width="76" height="16" rx="4" fill={isAutomated ? '#065f46' : '#15803d'} stroke={isAutomated ? '#34d399' : '#86efac'} strokeWidth="1.2" className="shadow" />
                  <text x={x} y={y - height - 12} textAnchor="middle" fill="#f0fdf4" fontSize="8.5" fontWeight="black" fontFamily="sans-serif">
                    {isAutomated ? 'GARRISON AUTO 🛡️' : 'CLAIM YIELD!'}
                  </text>
                </g>}

              {/* Unstaffed Worker Warning Floating Badge */}
              {isUnstaffed && !isReadyToHarvest && !plot.isUpgrading && <g className="animate-pulse pointer-events-none select-none">
                  <rect x={x - 36} y={y - height - 24} width="72" height="15" rx="4" fill="#451a03" stroke="#f59e0b" strokeWidth="1.2" className="shadow" />
                  <text x={x} y={y - height - 13} textAnchor="middle" fill="#fef08a" fontSize="7.5" fontWeight="black" fontFamily="monospace">
                    ⚠️ UNSTAFFED
                  </text>
                </g>}
            </g>;
      ...
      {activeNpcVillage && <div onClick={() => setActiveNpcVillage(null)} className="fixed inset-0 z-50 bg-black/60 md:backdrop-blur-xs flex items-center justify-center p-3 animate-in fade-in duration-150">
          <div onClick={e => e.stopPropagation()} className="w-full max-w-sm bg-gradient-to-b from-[#f5ebd6] via-[#ebdcc1] to-[#dfcba6] border-2 border-[#8c6843] rounded-3xl shadow-2xl p-4 text-[#442813] font-serif space-y-3">
            {/* Header */}
      ...
      {hoveredBuilding && <BuildingHoverTooltip buildingId={hoveredBuilding.id} level={hoveredBuilding.level || buildings[hoveredBuilding.id] || 1} resources={resources} technologies={technologies} troops={troops || {
      total: 20
    }} laborEfficiency={stats?.laborEfficiency || 1.0} faction={faction} mousePos={{
      x: hoveredBuilding.x,
      y: hoveredBuilding.y
    }} />}
      ...
      <div className="absolute top-2.5 right-2.5 z-30 flex flex-col items-center gap-1.5 pointer-events-auto select-none">
        ...
        sounds.playCoin();
        rotateKingdom?.();
      }} className="w-10 h-10 rounded-full bg-gradient-to-br from-[#f6ebd6] via-[#ebdcc1] to-[#dfcba6] border-2 border-[#8c6843] shadow-[0_4px_14px_rgba(0,0,0,0.55)] flex flex-col items-center justify-center text-[#442813] hover:brightness-110 active:scale-90 transition group cursor-pointer" title={`Rotate Kingdom Grid 90° (${rotationAngle}°) [R]`} aria-label="Rotate Citadel Grid">
          <div className="w-5 h-5 flex items-center justify-center transition-transform duration-500 ease-out" style={{
      ...
      }} className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#ebdcc1] to-[#dfcba6] border border-[#8c6843] shadow-md flex items-center justify-center text-sm font-black text-[#442813] hover:brightness-105 active:scale-90 transition cursor-pointer" title="Zoom In (+)" aria-label="Zoom In">
          +
        </button>
      ...
      }} className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#ebdcc1] to-[#dfcba6] border border-[#8c6843] shadow-md flex items-center justify-center text-sm font-black text-[#442813] hover:brightness-105 active:scale-90 transition cursor-pointer" title="Zoom Out (−)" aria-label="Zoom Out">
          −
        </button>
      </div>
    </div>;
