...
export const CONSTRUCTIBLE_BLUEPRINTS = ['farm', 'granary', 'lumber', 'quarry', 'barracks', 'watchtower', 'well', 'greenhouse', 'vault'];
...
export const calculateBuildingCost = (type, targetTier) => {
  const base = BASE_BUILDING_COSTS[type] || BASE_BUILDING_COSTS.farm || {
    gold: 35,
    wood: 15,
    stone: 0,
    flora: 0
...
export function calculateResourceCaps(buildings = {}, grid = null) {
  ...
  if (grid && Array.isArray(grid)) {
    ...
    const finalWaterCap = hasWell ? Math.max(300, waterCap + vaultWaterBonus) : Math.max(300, 300 + vaultWaterBonus);
    return {
...
export const AMORTIZATION_CONFIG = {
  COST_EXPONENT_ALPHA: 1.58,
  YIELD_EXPONENT_BETA: 1.36,
  TARGET_PROGRESSION_HOURS: 700
};