...
- **Troop Auto-Collection, Realm Calendar & Seasonal Weather Overhaul**:
  ...
  - **Zero-Bar Mobile Floating HUD, Floating Menu Trigger & Realm Settings Overhaul**:
    ...
    - **Floating Top-Left Chronometer & Resource Cluster (`MobileFloatingHUD.jsx`)**: Anchored at `fixed top-2.5 left-2.5 z-30 pointer-events-none md:hidden` with `pointer-events-auto` children. Upper row renders rounded translucent badge with condensed date, time, weather glyph (`1st Harv, 26 ADX • 08:00 ⛅`), and speed accelerator toggle (`1x/2x/5x`). Lower row renders tight cluster of compact resource badges (Gold, Food, Water, Wood, Stone) with abbreviated values and compact famine warning pill.
