...
* **Step 3:** **VALIDATION, LINT & REFACTORING RULE**: ALWAYS run `pnpm run lint` (or `pnpm eslint .`) BEFORE running `pnpm run build` to catch undefined variables (`no-undef`), broken references, and syntax errors early to preserve tokens and prevent WSOD/runtime crashes. Ensure all state hooks, event handlers, and procedural Web Audio nodes maintain strict lifecycle isolation. When refactoring components out of or within `App.jsx`, ensure zero circular dependencies, zero broken prop contracts, and zero audio context memory leaks.
...
* NEVER conclude an execution turn or build without running `pnpm run lint` (`pnpm eslint .`) to guarantee zero undefined references or syntax linter errors.
